//---------------------------------------------------------------------------
// Ar2uino - Arduino Reflow Oven Controller - v1.2k
// More info & FULL description: http://ar2uino.wordpress.com
//
// v1.2k = Orignal version 1.2, modified by "K"iritoTech
// Modified by KiritoTech 2019-11-17, to work with 1.8" TFT SPI ST7735 and custom PCB
// Work for Toaster oven
...
// Modified by KiritoTech 2019-11-17
//----------------------------------------------------------------------------
//#include <Helpers.h>
#include <EEPROM.h>
#include <SPI.h>
...
// Setup definitions
#define IPS 100 // Interrupts Per Second (Max=100)
...
#define MPPD 5 // Manual oven PPD - used ONLY for manual oven
byte PPD=20; // How many Percent (power) Per (Celcius) Degree
...
int boostProfile=99, boostStart=245, boostStop=280, boostPercent=90;
// Menue item data
...
void setup()
{
 ...
 // Set up serial com
 Serial.begin(115200);
 Serial.println(F("Ar2uino Reflow Oven v1.2k STARTED!"));
 Serial.println("");
 // FIRST TIME ONLY! Set up Oven Parameters and default profiles in EEPROM
 // Parameter: 0=First time only, 1=Always rewrite
 resetEEPROM(0); // To save 856 bytes: MOVE This whole function to a separate program!
...
//----------------------------------------------------------------------------
{
 ...
 tft.setTextColor(CYAN); tft.setCursor(40, y - 7); tft.print(pName);
...
// Date Programmer Action
// 190202 Sfenn ar2uino.wordpress.com original code v1.2
// 2019-11-17 KiritoTech adapted to work with ST7735 1.8"
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
{
 ...
 headings("Reflow: ", 53, 1, WHITE, BLUE);
 tft.print(pName);
 // Draw temp heading
 headings("Temp", 30, 33, LIGHT_GREY, BLACK);
 // Draw diff heading
 headings("Diff", 30, 61, LIGHT_GREY, BLACK);
 // Draw remain heading
 // Now shows remaining time to start cooling!
 headings("Remain", 30, 89, LIGHT_GREY, BLACK);
 // Draw Power heading
 headings("Pwr:", 23, 103, LIGHT_GREY, BLACK);
 // Draw TAL heading
 headings("TAL:", 24, 112, LIGHT_GREY, BLACK);
 // Start reflow from beginning
 nextMillis = 0;
 drawTempScale(CLEARBUFFER , 0, 0, 0, 0);
 ...
 drawTempScale(tempNow - profileTempNow , 60, 13, 120, 103);
 // Draw current temp centered
 tft.setTextScale(2);
 tft.setCursor(30, 22, REL_XY);
 tft.setTextColor(WHITE);
 if (tempNow > 183) tft.setTextColor(CYAN);
 tft.fillRect(10, 14, 40, 15, BLACK);
 tft.print(int(tempNow), DEC);
 // Draw diff temp centered with a separate minus sign
 tft.setTextScale(2);
 tft.fillRect(10, 42, 40, 15, BLACK);
 if (int(tempNow - profileTempNow) < 0) {
 tft. drawLine(10, 50, 17, 50, YELLOW);
 }
 tft.setCursor(30, 50, REL_XY);
 tft.setTextColor(YELLOW);
 tft.print(abs(int(profileTempNow - tempNow)), DEC);
 // Draw time remaining centered
 tft.setTextScale(2);
 tft.setCursor(30, 78, REL_XY);
 tft.setTextColor(GREEN);
 tft.fillRect(10, 70, 40, 15, BLACK);
 tft.print(profileCoolTime - (nowMillis / 1000), DEC);
 // Draw power centered
 tft.setTextScale(1);
 tft.setCursor(46, 103, REL_XY);
 tft.setTextColor(RED);
 tft.fillRect(36, 99, 18, 8, BLACK);
 tft.print(int(pPower), DEC);
 // Draw TAL centered
 tft.setTextScale(1);
 tft.setCursor(46, 112, REL_XY);
 tft.setTextColor(CYAN);
 tft.fillRect(36, 108, 18, 8, BLACK);
 tft.print(tal, DEC);
 }
...
void drawTempScale(int value, int x, int y, int w, int h)
//---------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
{
 ...
 // Lee's Reflow Profile (If temp is "0"=Will be set to current temp)
 char profile0Name[] = "Lee's";
 byte profile0Temp[] = { 25, 179, 185, 215, 30 }; // The temperature for each interval
 byte profile0StepTime[] = { 0, 220, 35, 30, 60 }; // The length in secs for interval
 // Standard Kester Reflow Profile (0=Will be set to current temp)
 char profile1Name[] = "Std";
 byte profile1Temp[] = { 25, 150, 175, 225, 225, 30};
 byte profile1StepTime[] = { 0, 90, 120, 20, 20, 60 };
